...
extension URLRequestBuilder {
    ...
    func handelError<T: BaseModel>(apiError: ApiError?, data: Any?, observer: AnyObserver<[T]>) {
        if let apiError = apiError {
            observer.onError(APIErrorModel(message: apiError.message, error: apiError.title, code: apiError.rawValue))
        } else if let err = data as? APIErrorModel{
            observer.onError(err)
        } else if let errorArr = data as? [APIErrorModel]{
            observer.onError(errorArr.first ?? APIErrorModel(message: apiError?.message ?? "", error: apiError?.title ?? "", code: apiError?.rawValue ?? 0 ))
        }
    }
}
